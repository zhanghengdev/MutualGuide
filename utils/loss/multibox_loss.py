...
class MultiBoxLoss(nn.Module):
    ...
    def forward(
        ...
        if self.mutual_guide:
            ...
            # print(torch.histc(overlap_t, bins=10, min=0.0, max=1.0))
            ...
            with torch.no_grad():
                conf_label = torch.FloatTensor(num * num_priors,
                        self.num_classes + 1).cuda().zero_()
                conf_label.scatter_(1, conf_t.view(-1, 1), 1)
                conf_label = conf_label[:, 1:]
            pos_idx = (pos
                       + neg).gt(0).unsqueeze(-1).expand_as(conf_data)
            conf_data = conf_data[pos_idx].view(-1, self.num_classes)
            conf_label = conf_label[pos_idx.view(-1,
                                    self.num_classes)].view(-1,
                    self.num_classes)
            loss_c = self.focal_loss(conf_data, conf_label,
                    reduction='sum')
            loss_c /= pos_cls_num
            ...
            with torch.no_grad():
                batch_label = torch.FloatTensor(num * num_priors,
                        self.num_classes + 1).cuda().zero_()
                batch_label.scatter_(1, conf_t.view(-1, 1), 1)
                batch_label = batch_label[:, 1:][posneg_idx.view(-1,
                        self.num_classes)].view(-1, self.num_classes)
            loss_c = self.focal_loss(conf_data, batch_label,
                    reduction='sum')
            loss_c /= pos_num
